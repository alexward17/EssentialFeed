...
private final class FeedCachePolicy {
    ...
    final func validate(_ timestamp: Date, againts date: Date) -> Bool {
...
extension LocalFeedLoader: FeedLoader {
    ...
    public func load(completion: @escaping ((LoadResult) -> Void)) {
        store.retrieve { [weak self] result in
            ...
            case let .found(feed, timestamp) where cachePolicy.validate(timestamp, againts: currentDate()):
...
extension LocalFeedLoader {
    public func validateCache() {
        store.retrieve { [weak self] result in
            ...
            case let .found(_, timestamp) where !cachePolicy.validate(timestamp, againts: currentDate()):
                store.deleteCachedFeed { _ in }
