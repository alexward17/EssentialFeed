...
class URLSessionHTTPClientTests: XCTestCase {
    ...
    func test_getFromURL_failsOnAllInvalidRepresentationCases() {
        ...
        XCTAssertNotNil(resultErrorFor((data: Self.anyData, response: nil, error: nil)))
        XCTAssertNotNil(resultErrorFor((data: Self.anyData, response: nil, error: Self.anyError)))
        XCTAssertNotNil(resultErrorFor((data: nil, response: nonHTTPURLResponse(), error: Self.anyError)))
        XCTAssertNotNil(resultErrorFor((data: nil, response: anyHTTPURLResponse(), error: Self.anyError)))
        XCTAssertNotNil(resultErrorFor((data: Self.anyData, response: nonHTTPURLResponse(), error: Self.anyError)))
        XCTAssertNotNil(resultErrorFor((data: Self.anyData, response: anyHTTPURLResponse(), error: Self.anyError)))
        XCTAssertNotNil(resultErrorFor((data: Self.anyData, response: nonHTTPURLResponse(), error: nil)))
    }
    ...
    private func resultFor(_ values: (data: Data?, response: URLResponse?, error: Error?)?, taskHandler: (HTTPClientTask) -> Void = { _ in },  file: StaticString = #file, line: UInt = #line) -> HTTPClient.Result {
        ...
        taskHandler(sut.get(from: Self.anyURL) { result in
